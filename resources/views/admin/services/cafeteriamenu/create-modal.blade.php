<div x-data="{
    ...
    monthCreate: '',
    plantelesCreate: [],
    handleFileInputCreate(event) {
        const file = event.target.files[0];
        this.fileCreate = file;
        this.fileNameCreate = file.name;
        this.fileSizeCreate = (file.size / 1024).toFixed(2) + ' KB';
    },
    handleFileDropCreate(event) {
        const file = event.dataTransfer.files[0];
        this.fileCreate = file;
        this.fileNameCreate = file.name;
        this.fileSizeCreate = (file.size / 1024).toFixed(2) + ' KB';
        this.$refs.fileInputCreate.files = event.dataTransfer.files;
    },
    removeFileCreate() {
        this.fileCreate = null;
        this.fileNameCreate = '';
        this.fileSizeCreate = '';
        this.$refs.fileInputCreate.value = null;
    },
    openModalCreate() {
    ...
    <div x-show="openCreate" x-cloak
        ...
        <div @click.away="openCreate = false" class="relative w-full max-w-md p-6 bg-white rounded-lg shadow-lg">
            <!-- Modal header -->
            ...
            <form action="{{ route('admin.menu-cafeteria.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div x-data="{ isDragging: false }" @dragover.prevent="isDragging = true"
                    ...
                    @drop.prevent="isDragging = false; handleFileDropCreate($event)">
                    <label for="pdf"
                    ...
                    <div class="relative flex items-center justify-center w-full p-4 border-2 border-dashed rounded-lg"
                        ...
                        <input x-ref="fileInputCreate" type="file" name="pdf" id="pdf"
                            ...
                            @change="handleFileInputCreate($event)">
                        <p class="text-gray-500">Arrastra el archivo aquí o haz clic para seleccionar</p>
                    </div>
                </div>

                <!-- Preview del archivo PDF -->
                <div x-show="fileCreate" class="mt-4 p-4 bg-gray-100 rounded-lg">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-900" x-text="fileNameCreate"></p>
                            <p class="text-xs text-gray-500" x-text="fileSizeCreate"></p>
                        </div>
                        <button type="button" @click="removeFileCreate" class="text-red-600 hover:text-red-800">
                ...
                <div>
                    <label for="school_cycle_id"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-600">Ciclo
                        Escolar</label>
                    <select name="school_cycle_id" id="school_cycle_id" x-model="schoolCycleCreate"
                ...
                <div>
                    ...
                    <select name="month" id="month" x-model="monthCreate"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">
                ...
                <div>
                    ...
                    <select name="plantel_ids[]" id="plantel_ids" x-model="plantelesCreate" multiple
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">
